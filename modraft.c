 ...
 * exactly once within a multithreaded program. This one does not have an election, rather just generation of random
 * number to indicate the epoch-leading thread will pass the batton to a next miner in the network.
 * COMPILER COMMAND IS gcc -pthread modraft.c -o modraft.out
...
int numepochs = 300000,votes=0,numblocks=200000,k=0,randsecret=rand();

while(globalepoch < numepochs && k < numblocks){

/* Create the new epoch assumes assignment of trusted leader (first thread) and random secret
*/
		pthread_mutex_lock(&mutexsum);
		gepochduration = (rand()/10000000)+10,globalepoch++,randsecret=rand();
/*		printf("Global Epoch Number: %i, Duration %i\n",globalepoch,gepochduration);
*/
		pthread_mutex_unlock(&mutexsum);}
/* End of epoch creation routine
*
